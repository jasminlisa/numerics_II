...
void generateKd(int dim, struct Matrix* kd){
	if (dim==1) {
		change_Matrix(kd,0,0,-2*(float)((dim+1)*(dim+1)));
	} else {
		change_Matrix(kd,0,0,-2*(float)((dim+1)*(dim+1)));
		change_Matrix(kd,0,1,1*(float)((dim+1)*(dim+1)));
		change_Matrix(kd,dim-1,dim-2,1*(float)((dim+1)*(dim+1)));
		change_Matrix(kd,dim-1,dim-1,-2*(float)((dim+1)*(dim+1)));
		for(int i=1;i<dim-1;i++){
			change_Matrix(kd,i,i-1,1*(float)((dim+1)*(dim+1)));
			change_Matrix(kd,i,i+1,1*(float)((dim+1)*(dim+1)));
			change_Matrix(kd,i,i,-2*(float)((dim+1)*(dim+1)));
		}
	}
}
